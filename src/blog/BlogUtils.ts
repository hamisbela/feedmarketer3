...
function fixDuplicateH1(content: string): string {
  ...
  // Replace any other occurrence of the same title with h1 formatting
  const escapedTitle = title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // Escape special regex chars
  const duplicateTitleRegex = new RegExp(`^#\\s+${escapedTitle}$`, 'gm');
  
  // Count occurrences
  const matches = [...content.matchAll(duplicateTitleRegex)];
  
  if (matches.length <= 1) {
    return content; // No duplicates, return unchanged
  }
  ...
  return fixedContent.replace(duplicateTitleRegex, (match) => {
